...
*/
  ...
  µb.tabContextManager = (function() {
    ...
    TabContext.prototype.update = function() {
      ...
      this.rootDomain =
        µb.URI.domainFromHostname(this.rootHostname) || this.rootHostname;
    };
  ...
  vAPI.tabs.onPopupUpdated = (function() {
    ...
    const popupMatch = function(openerURL, targetURL, popupType) {
      ...
      if (fctxt.getTabHostname() !== "" && targetURL !== "about:blank") {
        ...
        if (
          popupType === "popup" &&
          µb.sessionSwitches.evaluateZ("no-popups", fctxt.getTabHostname())
        ) {
          fctxt.filter = {
    ...
    return function(targetTabId, openerDetails) {
      ...
      if (
        µb.getNetFilteringSwitch(
          µb.normalizePageURL(openerTabId, openerURL)
        ) === false
      ) {
        return;
      }
  ...
  µb.updateToolbarIcon = (function() {
    ...
    let updateBadge = function(tabId) {
      ...
      if (
        window.store &&
        pageStore &&
        typeof pageStore.perLoadBlockedRequestCount === "number"
      ) {
        window.store.dispatch(
          window.actions.tabs.produce(state => {
            if (state[tabId]) {
